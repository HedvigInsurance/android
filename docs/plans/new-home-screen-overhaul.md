...
**DONE — test rot repaired** (commit `9869158`): `GetHomeUseCaseTest` stale flags → the single surviving `Feature.ENABLE_NEW_CONVERSATION_FROM_INBOX` (help-center is now hardcoded `true`, claim-history flag gone); `HomePresenterTest` `travelBannerInfo` → `addonBannerInfos = listOf()` and `CrossSellSheetData(..., recommendedAddon = null)`. `:feature-home` tests pass.

**DONE — Forever re-added as a bottom-nav tab** (commits `0b38c2d` revert + `98c02c8` pill drop). Key finding: the app **always had two Forever keys** — the top-level `feature-forever` `ForeverKey` (`TopLevelTabRoot`, the tab) and a non-top-level `feature-payments` `ForeverKey` (pushed in-flow from Discounts). So `git revert 1e0c3d8aae` correctly restored the original design (module, `TopLevelTab.Forever`, nav-bar, deep links, nav-serialization tests). `PaymentsDeepLinks.kt` resolved to the revert side; `shared/forever-ui/ForeverDestination.kt` restored to its pre-removal top-level form (`ForeverDestination(viewModel)`, no back button). Then dropped the **home Forever pill** (carousel is now 3 chips) and removed all `navigateToForever` wiring from `HomeDestination`/`HomeEntries`/`HedvigEntryProvider` (the payments `ForeverKey` + Discounts→Forever stay — that's the legitimate non-top-level path). Verified: `:app:compileDebugKotlin` + `:app`/`:feature-home` unit tests + nav-serialization tests + lint all green.

**(superseded) Original re-add plan:**
