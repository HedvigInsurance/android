...
- All build/test-verified (`:feature-home` tests + ktlint; `:app` compile + full unit tests + `assembleDebug`; `:forever-ui` + `:feature-payments` compile).

**Pre-existing finding (NOT from this work):** `no-wildcard-imports=true` is violated by 3 committed `Class.*` wildcard imports that predate this branch's home work and slipped past `ktlintFormat` (which can't auto-expand wildcards): `feature-payments/.../GetUpcomingPaymentUseCase.kt` (`PaymentConnection.*` — used by bare references, so needs a real expansion) and `feature-claim-chat/.../ClaimChatDestination.kt` (`ClaimChatEvent.*` + `…AudioRecording.*` — both used). These fail `lintKotlinMain`/`ktlintCheck` and will block the eventual PR (item 18); left untouched here since they're unrelated to the home/Forever work and require non-trivial, careful expansion.
