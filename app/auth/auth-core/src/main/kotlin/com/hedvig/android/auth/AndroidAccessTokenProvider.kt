...
internal class AndroidAccessTokenProvider(
  ...
  override suspend fun provide(): String? {
    ...
    val accessToken = (authTokenServiceProvider.provide().authStatus.value as? AuthStatus.LoggedIn)?.accessToken
    ...
    return mutex.withLock {
      val authTokens = authTokenServiceProvider.provide().getTokens() ?: return@withLock null.also {
      ...
      if (authTokens.refreshToken.expiryDate.isExpired()) {
        ...
        authTokenServiceProvider.provide().logoutAndInvalidateTokens()
        return@withLock null
      }
      logcat(LogPriority.VERBOSE) {
        "$requestId Access token expired, but not expired refresh token, refreshing tokens now"
      }
      val refreshedAccessToken = authTokenServiceProvider.provide().refreshAndGetAccessToken() ?: return@withLock null.also {
