...
import com.hedvig.android.apollo.ApolloOperationError.OperationError.Unathenticated.throwable
...
internal class OdysseyService(
  ...
  suspend fun uploadAudioRecordingFile(flowId: String, file: File): UploadAudioRecordingResult {
    ...
    val response: HttpResponse = httpClient
      .safePost("$baseUrl$flowId/audio-recording") {
        setBody(
      ...
      .fold(
        ifLeft = { error ->
          raise(
            when (error) {
              is NetworkError.IOError -> ErrorMessage(
